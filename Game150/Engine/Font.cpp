...
/*Math::ivec2 CS230::Font::MeasureText(std::string text) {
    ...
    {
        size.x += char_rects[i].top_right.x;
        if (size.y < (char_rects[i].bottom_left.y - char_rects[i].top_right.y))
        {
            size.y = char_rects[i].bottom_left.y - char_rects[i].top_right.y;
        }
...
Math::ivec2 CS230::Font::MeasureText(std::string text)
{
    ...
    int maxHeight = 0;
    for (char c : text) {
        Math::irect charRect = GetCharRect(c);
        totalWidth += charRect.Size().x;
        if (charRect.Size().y > maxHeight) {
            maxHeight = charRect.Size().y;
        }
    }
